...
class AuthRepository {
  ...
  Future<RegisterResponse> register(RegisterRequest request) async {
    ...
    print(
        "📡 Register API Response: ${response.statusCode}, Body: ${response.body}"); // Debug print
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      PreferencesHelper().saveUser(registerResponse.user);
      PreferencesHelper().saveToken(registerResponse.accessToken);
  ...
  Future<LoginResponse> login(LoginRequest request) async {
    ...
    if (response.statusCode == 200) {
      ...
      PreferencesHelper().saveUser(loginResponse.user);
      PreferencesHelper().saveToken(loginResponse.accessToken);
  ...
  Future<void> resetPassword(
    ...
    final response = await http.post(
      ...
      headers: {'Content-Type': 'application/json'},
      body:
          jsonEncode({'email': email, 'new_password': newPassword, 'otp': otp}),
    );
